...
class ToBeAdjectiveRule(SentenceRule):
    def apply(self, sentence: Sentence) -> bool:
        ...
        return issue_found
def get_head(sentence: Sentence, word: Word) -> Word:
    return next((w for w in sentence.words if w.index == word.head_index), None)


def get_subject(sentence: Sentence, word: Word) -> Word:
    if word.dependency == "aux":
        head = get_head(sentence, word)
        if head:
            word = head

    return next(
        (w for w in sentence.words
         if w.dependency == "nsubj" and w.head_index == word.index),
        None
    )
...
class PresentSVARule(SentenceRule):
    def apply(self, sentence: Sentence) -> bool:
        ...
        for word in sentence.words:
            ...
            if (
                word.pos not in {"VERB", "AUX"} or
                "VerbForm=Fin" not in str(word.morph) or
                "Tense=Pres" not in str(word.morph)
            ):
                continue
            ...
            if subject.is_singular() != word.is_singular():
                issue_found = True
                msg = (
                    f"'{subject.text}' requires the "
                    f"{'singular' if subject.is_singular() else 'plural'} "
                    f"form of '{word.text}'"
                )
...
class PastToBeSVARule(SentenceRule):
    def apply(self, sentence: Sentence) -> bool:
        ...
        for word in sentence.words:
            if (
                word.lemma.lower() != "be" or
                "Tense=Past" not in str(word.morph) or
                word.dependency == "aux"
            ):
                continue
            ...
            if subject.is_singular() != word.is_singular():
                issue_found = True
                correct = "was" if subject.is_singular() else "were"
                msg = f"In the past tense, '{subject.text}' should use '{correct}'"
                critic = Critic(msg, Critic.Severity.HIGH, Critic.Type.GRAMMAR)
                word.add_critic(critic)
...
class PerfectTenseSVARule(SentenceRule):
    def apply(self, sentence: Sentence) -> bool:
        ...
        for word in sentence.words:
            if (
                word.lemma.lower() != "have" or
                word.dependency != "aux" or
                "Tense=Pres" not in str(word.morph)
            ):
                continue
